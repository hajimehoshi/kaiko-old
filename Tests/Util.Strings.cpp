...
BOOST_AUTO_TEST_CASE(Util_Strings_UTF8ToUTF16LE) {
  {
    const std::wstring expected = L"foo";
    const std::wstring actual = Strings::UTF8ToUTF16LE("foo");
    BOOST_CHECK(expected == actual);
  }
  {
    const std::wstring expected = L"caf\u00e9";
    const std::wstring actual = Strings::UTF8ToUTF16LE("caf\xc3\xa9");
    BOOST_CHECK(expected == actual);
  }
  {
    const std::wstring expected = L"\u30c6\u30b9\u30c8";
    const std::wstring actual = Strings::UTF8ToUTF16LE("\xe3\x83\x86\xe3\x82\xb9\xe3\x83\x88");
    BOOST_CHECK(expected == actual);
  }
  {
